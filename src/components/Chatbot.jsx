...
export default function Chatbot() {
  ...
  const generateResponse = (input) => {
    ...
    if (text.includes("project") || text.includes("qiplo") || text.includes("chatbot") || text.includes("build") || text.includes("shipped") || text.includes("make") || text.includes("product")) {
      if (text.includes("qiplo") || text.includes("attrition") || text.includes("retention")) {
        return "Qiplo (qiplo.vercel.app) is Akshat's flagship AI Customer Retention SaaS. He managed the entire lifecycle: writing PRDs and feature specs, designing Figma user journeys, implementing webhooks (Salesforce, HubSpot, Stripe), constructing a SOC 2 audit tracker, and building a Tabular Hybrid Transformer ML model. It proves his ability to ship complex SaaS products from scratch.";
      }
      if (text.includes("chatbot") || text.includes("prompt")) {
        return "Akshat designed and optimized a Conversational AI Chatbot in 2025. He ran prompt engineering, conducted A/B testing on prompt styles to maximize resolution rate, mapped user journeys, and monitored telemetry KPIs (satisfaction scores, resolution rate, response time).";
      }
      return "Akshat has shipped several notable projects:\n\n1. **Qiplo** (qiplo.vercel.app): An AI Customer Retention SaaS featuring 8 modules, SHAP feature importance, and CRM webhook integrations.\n2. **AI Chatbot**: A prompt-engineered conversational interface optimized via A/B testing and KPI metrics.";
    }
