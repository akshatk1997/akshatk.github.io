...
import { 
  ...
  FileText, Map, Layers, Rocket, Terminal 
} from 'lucide-react';
...
export default function Skills() {
  ...
  const getSkillIcon = (name) => {
    ...
    // Product Core Icons
    if (n.includes('prd') || n.includes('spec')) return <FileText className="w-7 h-7 text-amber-500" />;
    if (n.includes('stories')) return <Users className="w-7 h-7 text-amber-500" />;
    if (n.includes('roadmap')) return <Map className="w-7 h-7 text-amber-500" />;
    if (n.includes('backlog') || n.includes('sprint')) return <Layers className="w-7 h-7 text-amber-500" />;
    if (n.includes('market') || n.includes('gtm')) return <Rocket className="w-7 h-7 text-amber-500" />;
    if (n.includes('devops') || n.includes('copado')) return <Terminal className="w-7 h-7 text-amber-500" />;
    ...
    if (n.includes('generative') || n.includes('genai')) return <Sparkles className="w-7 h-7 text-[#c084fc]" />;
    ...
    if (n.includes('sas') || n.includes('analytics') || n.includes('power')) return <BarChart3 className="w-7 h-7 text-[#c084fc]" />;
  ...
  const getCategoryHoverStyles = (category) => {
    switch (category) {
      case "Product Core":
        return "group-hover:shadow-[0_0_15px_rgba(245,158,11,0.25)] group-hover:border-amber-500/45 group-hover:bg-amber-500/5";
      case "Product Engineering":
        return "group-hover:shadow-[0_0_15px_rgba(129,140,248,0.25)] group-hover:border-[#818cf8]/45 group-hover:bg-[#818cf8]/5";
      case "AI & Data Science":
        return "group-hover:shadow-[0_0_15px_rgba(192,132,252,0.25)] group-hover:border-[#c084fc]/45 group-hover:bg-[#c084fc]/5";
      case "UX & Product Design":
        return "group-hover:shadow-[0_0_15px_rgba(251,113,133,0.25)] group-hover:border-[#fb7185]/45 group-hover:bg-[#fb7185]/5";
      default:
        return "group-hover:shadow-[0_0_15px_rgba(129,140,248,0.25)] group-hover:border-[#818cf8]/45 group-hover:bg-[#818cf8]/5";
    }
  };
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 4-Box Layout Symmetrical Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 reveal">
          {about.techStack.map((stack, sIdx) => (
            <div 
              key={sIdx} 
              className="group bg-white dark:bg-zinc-900/40 backdrop-blur-md rounded-[2rem] p-8 border border-zinc-200/80 dark:border-zinc-800/80 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col"
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 justify-items-center items-center flex-grow">
                {stack.items.map((skill, idx) => (
                  <div key={idx} className="flex flex-col items-center gap-2.5 group cursor-pointer">
                    <div className={`transition-all duration-300 group-hover:scale-110 p-3.5 bg-gray-50/50 dark:bg-zinc-950/60 rounded-2xl border border-zinc-200/50 dark:border-zinc-850 ${getCategoryHoverStyles(stack.category)}`}>
                      {getSkillIcon(skill)}
                    </div>
                    <span className="text-[10px] md:text-[11px] font-bold text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white transition-colors text-center max-w-[95px] leading-tight">
                      {skill}
                    </span>
                  </div>
                ))}
